...
void md_install_service() {
    ...
    SC_HANDLE hService = CreateService(
            hSCManager,
            "LLDPBeacon",                   /* name of service */
            "LLDP beacon service",            /* name to display */
...
void md_service_main(DWORD argc, char **argv) {
    hServiceStatus = RegisterServiceCtrlHandler(argv[0], (LPHANDLER_FUNCTION) md_service_control);
    if (hServiceStatus == 0) {
        return;
    }
...
void md_remove_service() {
    ...
    hService = OpenService(hSCManager, "LLDPBeacon", DELETE | SERVICE_STOP);
